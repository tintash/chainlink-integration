...
;; Error Transferring STXLINK Token
(define-constant ERROR_TRANSFER_STXLINK_TOKEN u301)
...
(define-fungible-token stxlink-token)


;; Get the token balance of the specified owner in base units
(define-read-only (get-balance (owner principal))
  (ok (ft-get-balance stxlink-token owner)))
...
(define-public (mint-tokens (mint-amount uint) (mint-to principal) )
  (begin
    ...
    (ft-mint? stxlink-token mint-amount mint-to)))
...
(define-public (burn-tokens (burn-amount uint) (burn-from principal) )
  (begin
    ...
    (ft-burn? stxlink-token burn-amount burn-from)))
...
(define-public (revoke-tokens (revoke-amount uint) (revoke-from principal) (revoke-to principal) )
  (begin
    ...
    (ft-transfer? stxlink-token revoke-amount revoke-from revoke-to)))
...
(define-public (transfer-and-call
                  ...
                  (transfer-callback <stxlink-transfer-trait>)
                  (callback <oracle-callback>))
  (begin
    (if (is-ok (transfer MINIMUM_PAYMENT tx-sender contract-address none))
      (match (contract-call?
        transfer-callback                 
        transfer-success
        MINIMUM_PAYMENT          
