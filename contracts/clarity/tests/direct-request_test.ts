...
Clarinet.test({
    name: "Ensure create requests ends with trasfer-fail when orignator has not enough stx-link tokens",
...
Clarinet.test({
    name: "Ensure create requests ends with fullfiling request when all checks are passed",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        const deployer = accounts.get("deployer")!;
        const wallet_1 = accounts.get("wallet_1")!;
        const wallet_2 = accounts.get("wallet_2")!;
